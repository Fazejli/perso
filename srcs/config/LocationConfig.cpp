...
LocationConfig::LocationConfig()
    : _path("/"),
      ...
      _cgiHandlers(),
      _clientMaxBodySize(static_cast<size_t>(-1)),
      _errorPages() {
}
...
const std::map<int, std::string>&              LocationConfig::getErrorPages() const        { return _errorPages; }
...
void LocationConfig::setClientMaxBodySize(size_t size) { _clientMaxBodySize = size; }
...
bool LocationConfig::hasClientMaxBodySize() const {
    return _clientMaxBodySize != static_cast<size_t>(-1);
}

std::string LocationConfig::getErrorPagePath(int code) const {
